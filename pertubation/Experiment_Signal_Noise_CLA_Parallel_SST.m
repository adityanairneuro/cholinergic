...
parfor idx = 1:numel(iValues)
    ...
    i_curr = iValues(idx);
    [CC_All, CS_All] = simulation_cholinergic_sst(i_curr)
...
meanCC = -mean(CC_iter,3)

imagesc(meanCC(:,:)*100,[-40,40]);
colormap(cm)
xlabel('mean Noise')
ylabel('mean Signal')
title('CC')
...
subplot(1,2,2)
meanCS = -mean(CS_iter,3)
imagesc(meanCS(:,:)*100,[-40,40]);
colormap(cm)
xlabel('mean Noise')
ylabel('mean Signal')
title('CS')
sgtitle('SST Silenced')
% cm = colormap(gca)
% save('cmapcla.mat','cm')